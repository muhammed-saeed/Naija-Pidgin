...
train_df_path = "/home/VD/cychang/ironside_roberta/ROBERTA-base-en/sentiment/sentiment_analysis_data/pidgin/pcm_train.csv"
test_df_path = "/home/VD/cychang/ironside_roberta/ROBERTA-base-en/sentiment/sentiment_analysis_data/pidgin/pcm_test.csv"

mapping = {"positive":2, "neutral":1, "negative":2 }

train_df = pd.read_csv(train_df_path)
print('Training data...')
train_df_text = train_df.text.astype(str).values.tolist()
train_df_label = train_df.label.map(mapping).tolist()
print (train_df.head())

test_df = pd.read_csv(test_df_path)
# test_df = test_df.label.map(mapping)
print('\nTesting data...')
test_df_text = test_df.text.astype(str).values.tolist()
test_df_label = test_df.label.map(mapping).tolist()
print (test_df.head())
...
config = config_class.from_pretrained("/home/VD/cychang/ironside_roberta/ROBERTA-base-en/Transformers/Pre-training_from_checkpoint/pretraining_from_scratch_train_eval/checkpoint-32768", num_labels=num_labels)

model = model_class.from_pretrained("/home/VD/cychang/ironside_roberta/ROBERTA-base-en/Transformers/Pre-training_from_checkpoint/pretraining_from_scratch_train_eval/checkpoint-32768", config=config)
print('Model=\n',model,'\n')

tokenizer = tokenizer_class.from_pretrained(tokenizer_name, do_lower_case=False)
print('Tokenizer=',tokenizer,'\n')

#preparing the testing and the training datasaets into formats of dataset for training
#
