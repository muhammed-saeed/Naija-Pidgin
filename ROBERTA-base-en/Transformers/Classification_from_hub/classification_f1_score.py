...
mapping = {"positive":[1,0,0], "neutral":[0,1,0], "negative":[0,0,1] }
...
def loss_fn(outputs, targets):
    return torch.nn.BCEWithLogitsLoss()(outputs, targets.to(torch.long))
    #since the target is float
...
def train(epoch):
    ...
    for _,data in enumerate(training_loader, 0):
        ...
        # one_hot_targets = one_hot(targets.to(torch.int64), num_classes=3)
