...
def build_corpus(corpus_name, scen_id, scen, scen_short, scen_par = None, 
  ...
  print('new urlshelves:', new_urls, (' ' if new_urls and os.path.exists(url_dir) else ' not ') + 'overwriting', corpus_name + '_urls/')
  ...
  with open(corpus_f,'w') as c:
    c.write(''.join(('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n\t',
        ...
        '" scenario="' + scen.replace('_', ' ') + '" short="' + scen_short.replace('_',' ') +
...
def get_all_corpora_gen(new_urls = True, new_qs = False, flag = 'mini1'):
  ...
  participants = ['fast_food'#_restaurant',
  ...
  for i in range(len(scenarios)):
    ...
    build_corpus(scenarios[i], ids[i], scenarios[i], shorts[i], participants[i], new_urls, new_qs, flag)
...
if __name__ == '__main__':
  ...
  if '-m' in argv:
    build_corpus('trial_muell_dog',  '66', 'feed_a_pet_dog', 
