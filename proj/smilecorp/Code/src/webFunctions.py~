...
def remove_scriptstyle(html):
  ...
  clean = ''
  p = re.compile('<((script)|(SCRIPT)).*?</((script)|(SCRIPT))>',re.DOTALL)
  ...
  html = clean
  clean = ''
  p = re.compile('<((style)|(STYLE)).*?</((style)|(STYLE))>',re.DOTALL)
  spans = []
  for i,mo in enumerate(p.finditer(html)):
    #print(i)
    #print(mo.group(0) + '\n\n')
...
def get_doc(url, log_f, debug = False):
  ...
  with open(log_f, 'a') as log:
    ...
    try:
      ...
      h = urlreq.urlopen(req,timeout = 10)
    except Exception as e:
    ...
    try:
      ...
      log.write(str(e) + '\n')
      with open(log_f[:-4] + '_jhtmls'+ log_f[-4:],'a') as jerrlog:
        jerrlog.write('\n\n' + url + '\n' + jhtml)
      log.write('jhtml in file:' + log_f[:-4] + '_jhtmls'+ log_f[-4:])
      log.write('\n\t' + url + '\n\n')
      #print('\n\n2\n',hr.title,'\n',hr.doc)
      errors += 'parsing;'
