...
class Bing(object):
    def __init__(self, app_id, timeout = None, market = 'None',loglevel=logging.INFO):
        self.timeout = timeout #C's addition
        self.app_id = app_id
        self.log_filename = 'log.log'
        self.end_point = 'http://api.search.live.net/json.aspx?Appid=%s&'%app_id 
        if market:
          self.end_point += 'Market=' + market + '&'
    ...
    def talk_to_bing(self, query, sources, extra_args={}):
        ...
        if self.timeout is not None:
          response = urllib.request.urlopen(final_url, timeout = self.timeout)
        else:
